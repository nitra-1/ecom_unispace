...
const PriceDetailsModal = ({ inquiry, onClose }) => {
  ...
  const estimatedPrice = JSON.parse(inquiry.estimatedPrice)
        ...
        >
          ...
          <div className="border-t border-gray-200 pt-4">
            <table className="w-full text-sm">
              <tbody>
                {Object.entries(estimatedPrice).map(([key, value]) => (
                  <tr key={key} className="border-b last:border-b-0">
                    <td className="py-2 pr-4 font-medium text-gray-600">
                      {formatKey(key)}
                    </td>
                    {/* --- This was the fix: wrapped value in a <td> --- */}
...
const DoorInquiry = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const inquiryForItems = ['door', 'windows']
    ...
    <>
      ...
      <div className="bg-white border border-gray-200 rounded shadow-sm">
        {loading && data.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {data.map((item) => {
                    const valueDetails = JSON.parse(item.valueDetails)
                    return (
                      <tr key={item.id}>
          ...
          //   </div>
          <EmptyComponent
            title={' No Door or Window Inquiry Yet'}
            description={
              'No Door or Window inquiries yet—create one to get started.'
            }
            alt={'empty_Add'}
      ...
      <PriceDetailsModal inquiry={selectedInquiry} onClose={handleCloseModal} />
    </>
  )
}

export default DoorInquiry
