...
const CityBulkUpload = ({
  ...
  const download = async () => {
    try {
      ...
      let downloadUrl = `${getBaseUrl()}City/downloadCity`;
      fetch(downloadUrl, {
        method: "GET",
        headers: headers,
      })
        ...
        .then((blob) => {
          const customFileName = `City.xlsx`;
 ...
 */
  ...
  const upload = async () => {
    if (!modalShow?.file) {
      const message = "Please select file";
      setModalShow({ ...modalShow, errors: [message] });
    ...
    try {
      ...
      const submitFormData = new FormData();
      submitFormData.append("file", modalShow?.file);
      ...
      if (resData?.code === 200) {
        ...
        setModalShow({
          ...modalShow,
          file: null,
          errors: [resData?.message || "Upload failed"],
        });
      }

      // showToast(toast, setToast, resData);
    } catch (err) {
      setLoading(false);
      setFileInputKey(Date.now())
      const errorMsg = err?.message || "Something went wrong";
      setModalShow({ ...modalShow, errors: [errorMsg] });
