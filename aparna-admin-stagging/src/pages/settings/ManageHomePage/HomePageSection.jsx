...
function HomePageSection() {
  ...
  const homePageName = searchParams.get('homePageName')
  ...
  const addTheme = async (themeId) => {
    try {
      setLoading(true)
      const response = await axiosProvider({
        method: 'POST',
        endpoint: 'HomePageTheme',
        data: { themeId: themeId, homePageId: parseInt(id) },
        location: location?.pathname,
        userId: userInfo?.userId
      })
      setLoading(false)
    ...
    try {
      ...
      let apiUrls = [
        {
          ...
          queryString: `?homepageId=${id}&HomePageFor=${homepageFor}&pageIndex=0&pageSize=0`,
          state: 'homePageSection'
        },
        {
          endpoint: 'ManageTheme',
          queryString: `?pageIndex=1&pageSize=25`,
          state: 'themeSection'
        }
      ]
      ...
      if (!allState?.allSectionDetails?.length) {
        apiUrls.push({
          ...
          queryString: `?layoutFor=${homepageFor}`,
      ...
      const fetchData = async () => {
        let homePageSection, allSectionDetails, sectionOption, themeSection
        ...
        setAllState((draft) => {
          draft.homePageSection = homePageSection
          draft.themeSection = themeSection
        })
        setLoading(false)
      }
      fetchData()
  ...
  useEffect(() => {
    fetchData()
    dispatch(setPageTitle(`Home Page : ${homePageName}`))
  }, [])
      ...
      <div>
        ...
        <Row className="justify-content-center align-items-start">
          {checkPageAccess(
            ...
            <Col
              md={3}
              className="mt-4 position-sticky"
              style={{ top: '80px' }}
            >
              <Accordion
                defaultActiveKey="0"
              ...
              >
                {!allState?.homePageSection?.hasOwnProperty('data') &&
                ...
                {allState?.homePageSection?.hasOwnProperty('data') &&
                  allState?.allSectionDetails?.length > 0 && (
                    <Accordion.Item
                      eventKey={allState?.allSectionDetails?.length + 1}
                      key={allState?.allSectionDetails?.length + 1}
                      className="rounded"
                    >
                      <Accordion.Header className="bold">
                        Theme
                      ...
                      <Accordion.Body>
                        <Row className="flex-wrap gy-3 align-items-center">
                          {allState?.themeSection?.map((data) => (
                            <Col onClick={() => addTheme(data?.id)}>
                              <div
                                role="button"
                                className="d-flex m-auto w-100 align-items-center justify-content-center gap-2 flex-column border p-3 rounded"
                              >
                                <img
                                  ...
                                  width="70px"
                                  className="img-fluid"
                                  src={`${process.env.REACT_APP_IMG_URL}${_manageThemeOptionImg_}${data?.image}`}
                                />
                                <span className="pv-accordian-item-desc">
                                  {splitStringOnCapitalLettersAndUnderscores(
                                    data.name
