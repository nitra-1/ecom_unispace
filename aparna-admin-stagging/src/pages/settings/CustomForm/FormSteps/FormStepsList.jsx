...
const FormStepsForm = React.lazy(() => import("./FormStepsForm.jsx"));
const FormStepsDetails = React.lazy(() => import("./FormStepsDetails.jsx"));
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await axiosProvider({
        method: "DELETE",
        endpoint: `ManageFormSteps?Id=${id}`,
  ...
  const fetchData = async () => {
    try {
      ...
      if (response?.status === 200) {
        setData(response);
        setAllState((draft) => {
          draft.formSteps.data = response?.data?.data?.map((item) => ({
            value: item?.id,
            label: item?.name,
          }));
          draft.formSteps.page = response?.data?.pagination?.pageCount;
          draft.formSteps.hasMore =
            response?.data?.pagination?.pageCount > 1 ? true : false;
          draft.formSteps.loading = false;
          draft.formSteps.searchText = "";
          draft.formSteps.hasInitialized = true;
        });
      }
    } catch {
      showToast(toast, setToast, {
