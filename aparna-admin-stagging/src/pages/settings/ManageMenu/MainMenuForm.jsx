...
const MainMenuForm = ({
  ...
  const onSubmit = async (values, resetForm) => {
    let dataOfForm = {
      ...
      HeaderMenuFor: activeToggle ?? 'Web'
    }
  ...
  const validationSchema = Yup.object().shape(
    {
      ...
      specificationIds: Yup.string().when('redirectTo', {
        is: (redirectTo) => redirectTo === 'Specification list',
        then: () =>
          Yup.number()
      ...
      customLink: Yup.string().when('redirectTo', {
        ...
        then: () => Yup.string().required('Custom link required'),
        otherwise: () => Yup.string().nullable()
      }),
  ...
  const fetchSpecData = async (specTypeId, specValueId) => {
    try {
      const response = await axiosProvider({
        method: 'GET',
      ...
      initialValues={
        editData
          ? {
              ...editData,
              image:
                editData?.image === 'null' || editData?.image === null
                  ? ''
                  : editData?.image,
              imageAlt:
                editData?.imageAlt === 'null' || editData?.imageAlt === null
                  ? ''
                  : editData?.imageAlt
            }
          : initialValues
                  ...
                  <div className="input-file-wrapper mb-3">
                    ...
                    <ReactSelect
                      ...
                      onChange={(e) => {
                        ...
                        } else if (e?.value === 'Book Appointment') {
                          setFieldValue('customLink', 'Book Appointment')
                        } else if (e?.value === 'Kitchen Appointment') {
                          setFieldValue('customLink', 'Kitchen Appointment')
                        } else if (e?.value === 'Wardrobe Appointment') {
                      ...
                      {values?.redirectTo === 'Custom link' ? (
                        ...
                        <>
                          <InfiniteScrollSelect
                            ...
                            setToast={setToast}
                            onChange={(e) => {
                              const apiPath = checkRedirectTo(
                                values?.redirectTo
                              )

                              setFieldValue(apiPath.field, e?.value)

                              if (apiPath.field === 'specificationIds') {
                                fetchSpecData(e.value, e.specTypeValueId)
                              }
                            }}
                            required={true}
