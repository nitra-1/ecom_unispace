...
const HSNBulkUpload = ({
  ...
  const download = async () => {
    try {
      ...
      let downloadUrl = `${getBaseUrl()}HSNCode/downloadHSN`
      fetch(downloadUrl, {
        method: 'GET',
        headers: headers
      })
        ...
        .then((blob) => {
          const customFileName = `HSNCode.xlsx`
  ...
  const upload = async () => {
    ...
    try {
      ...
      if (response?.data?.code === 200) {
        ...
        setModalShow({
          ...modalShow,
          errors: [response?.data?.data + ' ' + 'This HSN Code already exist']
        })
      } else {
        setFileInputKey(Date.now())
