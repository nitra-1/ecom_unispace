...
const TaxMappingForm = lazy(() => import("./TaxMappingForm.jsx"));
...
const TaxMapping = () => {
  ...
  let MapBy = [
    ...
    { value: "Same Union Territory", label: "Same Union Territory" },
    { value: "Different City", label: "Different City" },
    { value: "Different State", label: "Different State" },
    { value: "Different Union Territory", label: "Different Union Territory" },
    { value: "Different Country", label: "Different County" },
  ];
  ...
  const fetchDropDownData = async (id = null) => {
    if (id) {
      const response = await axiosProvider({
        method: "GET",
        endpoint: "TaxType/byParentId",
        queryString: `?parentId=${id}&pageIndex=1&pageSize=20`,
      });
      if (response?.status === 200) {
        setAllState((draft) => {
          draft["taxType"].data = response.data?.data?.map(
            selectOptionConfig?.taxType?.mapper
          );
        });
      }
    }
                      ...
                      <td className="text-center">
                        <div className="d-flex gap-2 justify-content-center">
                          {checkPageAccess(
                            ...
                            <span
                              onClick={() => {
                                allState?.tax?.data?.length === 0 &&
                                  fetchDropDownData(data?.taxId);
